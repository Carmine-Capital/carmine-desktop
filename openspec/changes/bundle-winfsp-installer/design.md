...
CloudMount on Windows depends on WinFsp (Windows File System Proxy) as a system-level filesystem driver. Currently, users must install WinFsp separately before CloudMount can mount drives. The app has a runtime preflight check that detects missing WinFsp and shows an error, but this still requires the user to leave the app, find WinFsp, install it, and restart.
...
**Goals:**
- Zero-friction WinFsp installation during CloudMount setup
- Skip WinFsp install if already present (idempotent)
- Abort CloudMount installation with clear message if WinFsp install fails
- Satisfy WinFsp redistribution license requirements

**Non-Goals:**
- WinFsp removal on CloudMount uninstall (other apps may depend on it)
...
**Alternative considered:** Interactive install (`/qr` or `/qb`) — adds unnecessary friction when the user already consented to the CloudMount install.
...
WinFsp is a shared system component. Other applications (Cryptomator, SSHFS-Win, rclone) may depend on it. Removing it during CloudMount uninstall could break those apps.
...
- **MSI install failure** → Show a message box explaining WinFsp is required, abort CloudMount install. The user can install WinFsp manually and retry.
