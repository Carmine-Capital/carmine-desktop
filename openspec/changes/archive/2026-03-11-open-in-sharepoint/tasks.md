...
- [x] 1.1 Add `web_url: Option<String>` field with `#[serde(rename = "webUrl")]` to `DriveItem` in `crates/cloudmount-core/src/types.rs`
- [x] 1.2 Add `webUrl` to the `$select` parameter in `list_children()` at `crates/cloudmount-graph/src/client.rs`
- [x] 1.3 Add `webUrl` to the `$select` parameter in `list_root_children()` at `crates/cloudmount-graph/src/client.rs`
...
- [x] 4.1 Add `tauri-plugin-deep-link` dependency to workspace `Cargo.toml` and `crates/cloudmount-app/Cargo.toml`
- [x] 4.2 Register the `cloudmount://` protocol scheme in the Tauri configuration (`tauri.conf.json` or equivalent)
- [x] 4.3 Add a deep-link event handler in the Tauri setup that parses `cloudmount://open-online?path=<encoded>`, decodes the path, and calls the same resolution + open logic as the `open_online` command
...
- [x] 5.1 Create a helper function to write registry keys under `HKCU\Software\Classes\*\shell\CloudMount.OpenInSharePoint` with the display text "Open in SharePoint" and command `cmd /c start cloudmount://open-online?path=%1`
...
- [x] 6.1 Create a `Open in SharePoint` shell script that reads `NAUTILUS_SCRIPT_SELECTED_FILE_PATHS`, percent-encodes the path, and calls `xdg-open "cloudmount://open-online?path=<encoded>"`
