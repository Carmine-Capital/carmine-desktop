...
- `crates/carminedesktop-app/dist/settings.html` — `loadMounts()` function and `<head>` CSP meta tag (primary fix).
- `crates/carminedesktop-app/dist/wizard.html` — `<head>` CSP meta tag (defensive hardening; no innerHTML data injection present but CSP is missing).
