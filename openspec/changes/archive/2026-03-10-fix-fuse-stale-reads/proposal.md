...
- A new notification bridge between `carminedesktop-cache` (where delta sync runs) and `carminedesktop-vfs` (where OpenFileTable lives) will be introduced — currently these crates have no communication path for open-handle awareness
...
- **Code**: `carminedesktop-vfs` (fuse_fs.rs, core_ops.rs) and `carminedesktop-cache` (sync.rs, manager.rs)
- **Cross-crate boundary**: Requires a new notification mechanism (callback or channel) from `carminedesktop-cache` → `carminedesktop-vfs` since delta sync and the open file table live in different crates
