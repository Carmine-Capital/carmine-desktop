## 1. DeltaSyncObserver trait in cloudmount-core

- [x] 1.1 Define `DeltaSyncObserver` trait in `crates/cloudmount-core/src/types.rs` with a single method `fn on_inode_content_changed(&self, ino: u64)`. The trait must be `Send + Sync` (for `Arc<dyn DeltaSyncObserver>`). Re-export from `crates/cloudmount-core/src/lib.rs`.

## 2. OpenFileTable extensions in cloudmount-vfs

- [x] 2.1 Add `stale: bool` field to `OpenFile` struct in `crates/cloudmount-vfs/src/core_ops.rs` (line 226-230). Initialize to `false` in `OpenFileTable::insert()`.
...
## 4. DeltaSyncObserver implementation in cloudmount-vfs
...
- [x] 4.3 Add a method to `CoreOps` (or `CloudMountFs`) to produce the `Arc<dyn DeltaSyncObserver>` — constructing `FuseDeltaObserver` with the shared `Arc<OpenFileTable>`.
...
- [x] 5.1 Add an optional `observer: Option<&dyn DeltaSyncObserver>` parameter to `run_delta_sync()` in `crates/cloudmount-cache/src/sync.rs`. Call `observer.on_inode_content_changed(inode)` inside the `if etag_changed` block (after line 118), alongside the existing disk removal and dirty-inode marking.
...
- [x] 5.3 Update `start_delta_sync()` in `crates/cloudmount-app/src/main.rs` (line 962) to construct the `FuseDeltaObserver` from the `CoreOps`/mount state and pass it to `run_delta_sync`. This is the wiring point where cache and VFS connect.
...
- [x] 6.1 Add unit tests for `OpenFileTable::get_content_size_by_ino` in `crates/cloudmount-vfs/tests/open_file_table_tests.rs`: test with Complete content, Streaming content, no matching handle, and multiple handles for same inode.
