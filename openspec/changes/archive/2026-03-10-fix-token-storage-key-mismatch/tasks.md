...
- [x] 1.1 Add `pub async fn finalize_sign_in(&self, id: &str) -> carminedesktop_core::Result<()>` to `AuthManager` in `crates/carminedesktop-auth/src/manager.rs`
...
- [x] 2.1 In `try_restore` in `crates/carminedesktop-auth/src/manager.rs`: after `load_tokens(account_id)` returns `None`, attempt `storage::load_tokens(&self.client_id)`
...
- [x] 3.1 In `complete_sign_in` in `crates/carminedesktop-app/src/commands.rs` (line 130): replace `state.auth.set_account_id(&drive.id).await` with `state.auth.finalize_sign_in(&drive.id).await.map_err(|e| e.to_string())?`

## 4. Tests

- [x] 4.1 Add test `finalize_sign_in_migrates_tokens_from_client_id_to_account_id` in `crates/carminedesktop-auth/tests/`: store tokens under client_id, call `finalize_sign_in(account_id)`, assert tokens exist under account_id and not under client_id
