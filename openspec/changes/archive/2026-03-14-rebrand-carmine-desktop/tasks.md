...
- [x] 1.1 Rename all 6 crate directories from `carminedesktop-*` to `carminedesktop-*`
...
- [x] 2.1 Replace all `carminedesktop_` / `carminedesktop-` references in `use` statements, module paths, and string literals across all `.rs` files
- [x] 2.2 Update env var references from `carminedesktop_*` to `CARMINEDESKTOP_*` (option_env!, env::var, .env parsing)
...
- [x] 6.2 Update WinFsp MSI resource path from `carminedesktop-app` to `carminedesktop-app`
...
- [x] 8.5 Grep for any remaining `carminedesktop` / `carminedesktop` references (excluding openspec/)
