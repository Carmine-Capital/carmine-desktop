...
- [x] 1.1 Read `crates/carminedesktop-vfs/src/cfapi.rs` and locate the `fetch_data` method
...
- [x] 3.1 Run `cargo build -p carminedesktop-vfs --target x86_64-pc-windows-msvc` (or cross-compile check) to confirm the change compiles with no warnings
