...
1. `carminedesktop.OpenOnline` with label "Open Online (SharePoint)" and command `carminedesktop://open-online?path=%1`
2. `carminedesktop.OpenLocally` with label "Open Locally" and command that opens the file via the default system handler
...
- **THEN** the system registers both `carminedesktop.OpenOnline` and `carminedesktop.OpenLocally` under `HKCU\Software\Classes\*\shell\`
...
The system SHALL remove both `carminedesktop.OpenOnline` and `carminedesktop.OpenLocally` registry entries only when the active mount count transitions from 1 to 0.
