...
**Goals:**
- Rename all 6 crate packages from `filesync-*` to `carminedesktop-*`
- Rename all 6 crate directories from `crates/filesync-*` to `crates/carminedesktop-*`
- Update all Rust module paths (`filesync_*` → `carminedesktop_*`)
- Update all user-facing strings, config paths, and service names from `filesync`/`FileSync` to `carminedesktop`/`carminedesktop`
...
**Non-Goals:**
- Automated migration of existing user config directories (`~/.config/filesync/` → `~/.config/carminedesktop/`) — pre-v1, not needed
...
- Changing the Git repository name or remote URL — already named carminedesktop
...
| Old | New | Where |
|-----|-----|-------|
| `filesync-*` | `carminedesktop-*` | Crate names, Cargo.toml, directory names |
| `filesync_*` | `carminedesktop_*` | Rust module paths (`use` statements, types) |
| `"filesync"` | `"carminedesktop"` | Keyring service name, config dir joins, FSName, DB name |
| `"FileSync"` | `"carminedesktop"` | DEFAULT_APP_NAME, PROVIDER_NAME, display strings, docs |
| `"filesync-tray"` | `"carminedesktop-tray"` | Tray icon ID |
| `com.filesync.*` | `com.carminedesktop.*` | Tauri identifier, macOS LaunchAgent, macOS plist |
| `filesync.service` | `carminedesktop.service` | systemd unit file |
| `filesync.db` | `carminedesktop.db` | SQLite database filename |
| `FileSyncFs` | `carminedesktopFs` | FUSE filesystem struct |
| `FileSyncCfFilter` | `carminedesktopCfFilter` | CfApi filter struct |
...
1. `git mv` crate directories (`crates/filesync-*` → `crates/carminedesktop-*`)
...
- Add `pub use carminedesktop_core as filesync_core;` in a compatibility crate → unnecessary complexity for pre-v1 software with no published crates on crates.io
...
**Decision**: The encrypted token fallback password changes from `"filesync-fallback-{user}@{home}"` to `"carminedesktop-fallback-{user}@{home}"`.
