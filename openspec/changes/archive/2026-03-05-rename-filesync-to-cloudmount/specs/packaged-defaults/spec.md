...
- **THEN** the OAuth2 flow uses this client ID instead of the generic carminedesktop app registration

#### Scenario: Custom app name
- **WHEN** `build/defaults.toml` contains `[branding]` with `app_name`
- **THEN** the system tray tooltip, window titles, notification titles, and wizard header all display the custom app name instead of "carminedesktop"

#### Scenario: No branding configured
- **WHEN** `build/defaults.toml` does not contain a `[branding]` section
- **THEN** the application uses "carminedesktop" as the default app name everywhere
