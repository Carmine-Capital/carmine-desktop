...
- [x] 1.1 Add `clap` (with `derive` feature) to workspace `[workspace.dependencies]` in root `Cargo.toml`, then add `clap = { workspace = true }` to `carminedesktop-app/Cargo.toml`
- [x] 1.2 Add `dotenvy` to workspace `[workspace.dependencies]` in root `Cargo.toml`, then add `dotenvy = { workspace = true }` to `carminedesktop-app/Cargo.toml`
- [x] 1.3 Add `.env` and `build/defaults.toml` to `.gitignore`
- [x] 1.4 Create `.env.example` in repo root with documented `carminedesktop_CLIENT_ID`, `carminedesktop_TENANT_ID`, `carminedesktop_APP_NAME`, `carminedesktop_LOG_LEVEL`, `carminedesktop_CONFIG` variables
- [x] 1.5 Rename `build/defaults.toml` to `build/defaults.toml.example` (git mv)
- [x] 1.6 Create `crates/carminedesktop-app/build.rs`: if `build/defaults.toml` doesn't exist, copy from `build/defaults.toml.example`; emit `cargo::rerun-if-changed` for the file

## 2. CLI Argument Parsing

- [x] 2.1 Define `CliArgs` struct with clap derive in `carminedesktop-app/src/main.rs`: `--client-id` (env=carminedesktop_CLIENT_ID), `--tenant-id` (env=carminedesktop_TENANT_ID), `--config` (env=carminedesktop_CONFIG), `--log-level` (env=carminedesktop_LOG_LEVEL), `--headless` flag
...
- [x] 3.1 Add `option_env!()` constants in `carminedesktop-app/src/main.rs`: `BUILD_CLIENT_ID`, `BUILD_TENANT_ID`, `BUILD_APP_NAME` for `carminedesktop_CLIENT_ID`, `carminedesktop_TENANT_ID`, `carminedesktop_APP_NAME`
...
- [x] 4.1 Create `RuntimeOverrides` struct in `carminedesktop-app/src/main.rs` with `client_id: Option<String>` and `tenant_id: Option<String>` populated from CLI args
...
- [x] 5.2 Implement placeholder client ID check: if client_id equals `00000000-0000-0000-0000-000000000000`, print actionable error to stderr (mention `docs/azure-ad-setup.md`, `--client-id`, `carminedesktop_CLIENT_ID`, `.env`) and exit(1)
...
- [x] 6.1 Add display detection helper in `carminedesktop-auth/src/oauth.rs`: on Linux check `$DISPLAY` and `$WAYLAND_DISPLAY`; on macOS/Windows always return true
...
- [x] 7.1 Create `docs/org-build-guide.md` with: overview of the config overlay pattern, step-by-step GitLab private repo setup (defaults.toml + .gitlab-ci.yml), step-by-step GitHub private repo setup (defaults.toml + .github/workflows/build.yml), CI variable configuration (CLIENT_ID masked, TENANT_ID, APP_NAME), how to update to a new carminedesktop version (change version tag)
