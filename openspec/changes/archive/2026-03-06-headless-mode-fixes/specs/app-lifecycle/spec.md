...
- **THEN** the system restores tokens, runs crash recovery for pending writes as a background task, starts all enabled mounts, starts the periodic delta sync loop, and logs "carminedesktop headless mode running — N mount(s) active"
