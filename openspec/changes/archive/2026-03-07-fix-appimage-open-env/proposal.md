...
- Extract a `pub(crate) fn open_with_clean_env(path: &str) -> Result<(), String>` helper inside `carminedesktop-app` that strips `LD_LIBRARY_PATH` and `LD_PRELOAD` on Linux before spawning `xdg-open`, and falls back to `open::that()` on other platforms.
...
- **Files changed**: `crates/carminedesktop-app/src/main.rs`, `crates/carminedesktop-app/src/tray.rs`
- **No API or crate boundary changes** — helper is `pub(crate)`, internal to carminedesktop-app
