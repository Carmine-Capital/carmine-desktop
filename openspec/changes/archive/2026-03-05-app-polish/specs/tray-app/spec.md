...
- **WHEN** mount state changes (mount started, stopped, added, removed, toggled, or authentication state changes)
- **THEN** the system SHALL rebuild the tray context menu to reflect the current mount list and status; each mount entry displays the mount name and its current state (Mounted, Unmounted, or Error)
