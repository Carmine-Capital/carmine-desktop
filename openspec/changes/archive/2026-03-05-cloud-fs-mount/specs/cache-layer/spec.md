...
The system SHALL buffer file writes locally and upload them asynchronously.

#### Scenario: Write buffered locally
- **WHEN** an application writes data to a mounted file
- **THEN** the system writes the data to a local buffer file in the cache directory and returns success to the caller without waiting for upload
...
- **THEN** the system uploads the buffered content to the Graph API and blocks until the upload completes
