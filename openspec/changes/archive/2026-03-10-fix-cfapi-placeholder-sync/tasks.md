## 1. Delta sync result type (`cloudmount-cache`)

- [x] 1.1 Define `DeltaSyncResult` struct in `crates/cloudmount-cache/src/sync.rs` with fields: `changed_items: Vec<DriveItem>` (items whose eTag changed), `deleted_items: Vec<DeletedItemInfo>` (deleted item ID + name + parent path captured before cache removal). Define `DeletedItemInfo` struct with `id: String`, `name: String`, `parent_path: Option<String>`.
...
- [x] 1.3 Export `DeltaSyncResult` and `DeletedItemInfo` from `cloudmount-cache` crate root (`lib.rs`).

## 2. Path resolution helper (`cloudmount-cache` or `cloudmount-core`)

- [x] 2.1 Add a helper function `resolve_relative_path(item: &DriveItem) -> Option<PathBuf>` that extracts the parent path from `item.parent_reference.path` (stripping the `/drive/root:` or `/drives/{id}/root:` prefix), joins it with `item.name`, and returns the relative path. Place in `cloudmount-cache/src/sync.rs` (private) or as a utility in `cloudmount-core` if reusable.
...
## 3. Placeholder update function (`cloudmount-vfs`)

- [x] 3.1 Add a new public function `apply_delta_placeholder_updates` in `crates/cloudmount-vfs/src/cfapi.rs`, gated with `#[cfg(target_os = "windows")]`. Signature: `pub fn apply_delta_placeholder_updates(mount_path: &Path, changed: &[(PathBuf, DriveItem)], deleted: &[PathBuf], writeback: &WriteBackBuffer, drive_id: &str)`.
...
- [x] 3.4 Make `item_to_metadata` a module-level function (currently `CloudMountCfFilter::item_to_metadata`) or extract it as a standalone `pub(crate)` function so the new `apply_delta_placeholder_updates` can use it without access to the filter struct.
- [x] 3.5 Export `apply_delta_placeholder_updates` from `cloudmount-vfs` crate root with appropriate `#[cfg]` gate.

## 4. Wire notification bridge (`cloudmount-app`)

- [x] 4.1 Update the `start_delta_sync` loop in `crates/cloudmount-app/src/main.rs` to capture the `DeltaSyncResult` from `run_delta_sync`. On `Ok(result)`, if there are changed or deleted items, resolve paths and call `apply_delta_placeholder_updates`.
...
- [x] 5.1 Add a method `has_pending(&self, drive_id: &str, item_id: &str) -> bool` to `WriteBackBuffer` in `crates/cloudmount-cache/src/writeback.rs` that checks if a pending entry exists for the given drive+item without loading content. Use `tokio::fs::try_exists` on the expected path.
