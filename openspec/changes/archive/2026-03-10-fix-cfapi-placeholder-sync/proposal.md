...
- Add a callback/notification mechanism so `run_delta_sync` (in `carminedesktop-cache`) can inform `carminedesktop-vfs` about items that changed or were deleted, without creating a direct crate dependency from cache → vfs.
...
- Wire the notification bridge in the app orchestration layer (`carminedesktop-app`) where both caches and mount handles are accessible.
...
- **Crates affected**: `carminedesktop-cache` (sync return type), `carminedesktop-vfs` (new public function for placeholder updates), `carminedesktop-app` (wiring the notification bridge in `start_delta_sync`).
- **Platform scope**: Windows only (`#[cfg(target_os = "windows")]`). FUSE backends are unaffected.
- **APIs**: `run_delta_sync` signature changes from `Result<()>` to `Result<DeltaSyncResult>` containing changed/deleted item lists. New public function in `carminedesktop-vfs` for applying placeholder updates given a mount path and list of changes.
