...
- [x] 1.2 Wire the Service Menu action to invoke a carminedesktop helper script with selected file paths
...
- [x] 2.2 Percent-encode each absolute path and build `carminedesktop://open-online?path=<encoded>` URLs
...
- [x] 3.2 Document prerequisites and troubleshooting (carminedesktop running, deep-link registration, `xdg-open` availability)
...
- [ ] 4.3 Validate a path outside carminedesktop mounts and confirm user-visible error notification
