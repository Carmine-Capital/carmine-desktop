...
- [x] 1.1 Add `display_name: &str` and `icon_path: &str` parameters to `register_sync_root()` in `crates/carminedesktop-vfs/src/cfapi.rs`
...
## 5. Update caller in `carminedesktop-app`

- [x] 5.1 In `crates/carminedesktop-app/src/main.rs` `start_mount()` (Windows path), pass `mount_config.name.clone()` as the new `display_name` argument to `CfMountHandle::mount()`

## 6. Update integration tests

- [x] 6.1 Update `CfMountHandle::mount()` call sites in `crates/carminedesktop-vfs/tests/cfapi_integration.rs` to pass a `display_name` argument (e.g., `"Test Mount"`)
- [x] 6.2 Update `CfMountHandle::mount()` call site in `crates/carminedesktop-app/tests/integration_tests.rs` to pass a `display_name` argument
