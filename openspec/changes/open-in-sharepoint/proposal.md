...
- Register a `cloudmount://` deep-link protocol so external tools (shell context menus, scripts) can trigger "Open in SharePoint" for a given path.
...
- **cloudmount-core**: `DriveItem` struct gains one `Option<String>` field.
- **cloudmount-graph**: `$select` parameter in `list_children` / `list_root_children` gains `webUrl`. No new API calls.
- **cloudmount-app**: New Tauri command, deep-link handler registration, platform-specific shell integration setup.
- **cloudmount-vfs**: No changes — `CoreOps::resolve_path` already returns `DriveItem`, which will now carry `webUrl`.
