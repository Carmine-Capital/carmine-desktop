...
- [x] 1.1 Add `cleanup_stale_mount(path: &str) -> bool` function in `crates/cloudmount-vfs/src/mount.rs`, platform-gated for Linux/macOS. Stat the path; if ENOTCONN (107) or EIO (5), attempt `fusermount3 -u`, then `fusermount -u` (Linux), or `umount` (macOS). Return true if cleanup succeeded or path was not stale.
...
- [x] 3.1 Add a unit test in `crates/cloudmount-vfs/tests/` for `cleanup_stale_mount` with a non-existent path (should return true — no stale mount).
