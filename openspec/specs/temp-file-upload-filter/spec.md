...
The transient check SHALL be a pure function of the filename only (no filesystem state, no item metadata). The check SHALL be applied in `CoreOps::flush_inode` (upload suppression) and in the CollabGate guard within `CoreOps::open_file` (preventing spurious online-open attempts for lock files). Both FUSE and WinFsp backends benefit uniformly from both call sites.
...
- **AND** the file opens locally without triggering an online-open attempt
