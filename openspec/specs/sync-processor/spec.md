...
The SyncProcessor provides an async upload pipeline for the VFS layer. It receives flush requests from FUSE/WinFsp backends, debounces rapid saves, manages concurrent uploads with bounded concurrency, retries failures with exponential backoff, and supports graceful shutdown and crash recovery. This replaces inline synchronous uploads with a decoupled, observable upload processor.
