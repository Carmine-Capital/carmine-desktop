...
use cloudmount_vfs::retry_pending_writes_for_drive;
use serde_json::json;
...
async fn test_pending_retry_recovers_after_transient_failure() {
    ...
    let test_id = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let base = std::env::temp_dir().join(format!("cloudmount-pending-retry-{test_id}"));
    let cache_dir = base.join("cache");
    let db_path = base.join("metadata.db");
    std::fs::create_dir_all(&cache_dir).unwrap();

    let cache = CacheManager::new(cache_dir, db_path, 100_000_000, Some(300)).unwrap();
