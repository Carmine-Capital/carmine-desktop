...
impl FileSystemContext for CloudMountWinFsp {
    ...
    fn get_volume_info(&self, volume_info: &mut VolumeInfo) -> winfsp::Result<()> {
        ...
        volume_info.set_volume_label("CloudMount");
        Ok(())
    }
...
impl WinFspMountHandle {
    ...
    pub fn mount(
        ...
        let mut host = FileSystemHost::new(volume_params, fs).map_err(|e| {
            cloudmount_core::Error::Filesystem(format!("WinFsp host creation failed: {e:?}"))
        })?;

        // 7. Mount at mountpoint.
        host.mount(mountpoint).map_err(|e| {
            cloudmount_core::Error::Filesystem(format!("WinFsp mount failed: {e:?}"))
        })?;

        // 8. Start.
        host.start().map_err(|e| {
            cloudmount_core::Error::Filesystem(format!("WinFsp start failed: {e:?}"))
        })?;
