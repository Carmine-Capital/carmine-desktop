...
- **Machine password**: `carminedesktop-fallback-{USER}@{config_dir}` — tied to user+machine.
- **Zeroization**: `zeroize` crate for key material.
- **Keyring service name**: `carminedesktop`.
- **Token path**: `{config_dir}/carminedesktop/tokens_{account_id}.enc`.
