...
impl VfsError {
    /// Map a `carminedesktop_core::Error` to a specific `VfsError` variant.
    pub fn from_core_error(e: carminedesktop_core::Error) -> Self {
        match &e {
            ...
            carminedesktop_core::Error::Network(_) => VfsError::TimedOut,
            _ => VfsError::IoError(e.to_string()),
        }
    }
}
...
impl CoreOps {
    ...
    fn handle_collab_gate_fallback(&self, ino: u64, caller_pid: Option<u32>, path: &str) {
        ...
        let is_collab = carminedesktop_core::open_online::is_collaborative(&ext);
