...
impl CarmineDesktopFs {
    ...
    pub fn mount(
        ...
        mountpoint: &str,
        auto_unmount: bool,
    ) -> carminedesktop_core::Result<fuser::BackgroundSession> {
        let mut config = Config::default();
        config.mount_options = vec![
            MountOption::RW,
            MountOption::FSName("carminedesktop".to_string()),
        ...
        fuser::spawn_mount2(self, mountpoint, &config)
            .map_err(|e| carminedesktop_core::Error::Filesystem(format!("mount failed: {e}")))
    }
...
impl Filesystem for CarmineDesktopFs {
