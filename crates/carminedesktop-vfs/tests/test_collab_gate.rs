...
use carminedesktop_core::types::{CollabOpenResponse, DriveItem, FileFacet, ParentReference};
use carminedesktop_graph::GraphClient;
use carminedesktop_vfs::core_ops::{CoreOps, VfsEvent};
...
fn unique_cache_dir(prefix: &str) -> std::path::PathBuf {
    let id = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    std::env::temp_dir().join(format!("carminedesktop-collab-{prefix}-{id}"))
}
...
fn collab_config_for_test_process() -> CollaborativeOpenConfig {
    ...
    if let Some(name) = carminedesktop_vfs::process_filter::current_process_name() {
        extra.push(name);
    }
...
async fn collab_gate_open_online_returns_redirect_error() {
    ...
    assert!(
        matches!(err, carminedesktop_vfs::core_ops::VfsError::CollabRedirect),
