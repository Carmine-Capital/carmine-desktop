...
Virtual filesystem exposing OneDrive/SharePoint via WinFsp on Windows. The shared logic lives in `core_ops.rs`; `winfsp_fs.rs` implements the WinFsp `FileSystemContext` trait and delegates to it.
...
- WinFsp `FileSystemContext` trait methods are sync. Bridge to async via `rt.block_on()`.
- Map `VfsError` variants to the appropriate `NTSTATUS` in the backend (e.g. `NotFound` → `STATUS_OBJECT_NAME_NOT_FOUND`).
...
- Do NOT make `FileSystemContext` trait methods async — WinFsp requires sync.
