...
// ---------------------------------------------------------------------------
// Collaborative Open config
// ---------------------------------------------------------------------------

let _collabConfig = { enabled: true, default_action: 'ask', timeout_seconds: 15, extensions: [] };

async function loadCollabConfig() {
  try {
    _collabConfig = await invoke('get_collab_config');
    document.getElementById('collab-enabled').checked = _collabConfig.enabled;
    document.getElementById('collab-default-action').value = _collabConfig.default_action;
    updateCollabVisibility();
    renderExtPrefs();
  } catch (e) {
    console.error(e);
    showStatus('Failed to load collaborative editing settings', 'error');
  }
}

function updateCollabVisibility() {
  const enabled = document.getElementById('collab-enabled').checked;
  document.getElementById('collab-options').style.display = enabled ? 'block' : 'none';
}

async function saveCollabConfig() {
  try {
    await invoke('update_collab_config', {
      enabled: _collabConfig.enabled,
      defaultAction: _collabConfig.default_action,
      timeoutSeconds: _collabConfig.timeout_seconds,
      extensions: _collabConfig.extensions,
    });
    showStatus('Collaborative editing settings saved', 'success');
  } catch (e) {
    showStatus(formatError(e), 'error');
  }
}
...
function renderExtPrefs() {
  const list = document.getElementById('ext-pref-list');
  list.innerHTML = '';
  if (_collabConfig.extensions.length === 0) {
    const empty = document.createElement('li');
    empty.className = 'ext-pref-empty';
    empty.textContent = 'No per-extension preferences set';
    list.appendChild(empty);
    return;
  }
  _collabConfig.extensions.forEach(function(pref) {
    const li = document.createElement('li');
    li.className = 'ext-pref-item';

    const nameSpan = document.createElement('span');
    nameSpan.className = 'ext-name';
    nameSpan.textContent = '.' + pref.ext;

    const actionSpan = document.createElement('span');
    actionSpan.className = 'ext-action';
    actionSpan.textContent = pref.action === 'online' ? 'Open online' : 'Open locally';

    const removeBtn = document.createElement('button');
    removeBtn.className = 'btn-sm btn-danger';
    removeBtn.textContent = 'Remove';
    removeBtn.addEventListener('click', function() {
      _collabConfig.extensions = _collabConfig.extensions.filter(function(e) {
        return e.ext !== pref.ext;
      });
      renderExtPrefs();
      saveCollabConfig();
    });
...
document.getElementById('collab-enabled').addEventListener('change', function() {
  _collabConfig.enabled = this.checked;
  updateCollabVisibility();
  saveCollabConfig();
});

document.getElementById('collab-default-action').addEventListener('change', function() {
  _collabConfig.default_action = this.value;
  saveCollabConfig();
});

document.getElementById('btn-add-ext-pref').addEventListener('click', function() {
  const extInput = document.getElementById('ext-pref-ext');
  const actionSelect = document.getElementById('ext-pref-action');
  const ext = extInput.value.trim().replace(/^\./, '').toLowerCase();
  if (!ext) {
    showStatus('Please enter a file extension', 'error');
    return;
  }
  // Remove existing entry for this extension if present
  _collabConfig.extensions = _collabConfig.extensions.filter(function(e) {
    return e.ext !== ext;
  });
  _collabConfig.extensions.push({ ext: ext, action: actionSelect.value });
  extInput.value = '';
  renderExtPrefs();
  saveCollabConfig();
});
...
loadCollabConfig().catch(e => showStatus(formatError(e), 'error'));
