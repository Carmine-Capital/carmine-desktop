...
//!
//! The previous default handler is saved so that files NOT on a CloudMount
...
///
/// Checks (in order):
/// 1. Saved `CloudMount.PreviousHandler` value
...
mod linux {
    ...
    const DESKTOP_FILE_NAME: &str = "cloudmount-open.desktop";
    ...
    /// Path to our .desktop file: ~/.local/share/applications/cloudmount-open.desktop
    fn desktop_file_path() -> cloudmount_core::Result<PathBuf> {
        dirs::data_dir()
            .map(|d| d.join("applications").join(DESKTOP_FILE_NAME))
            .ok_or_else(|| {
                cloudmount_core::Error::Config(
                    "no data directory available (dirs::data_dir returned None)".into(),
                )
            })
    }

    /// Path to our previous handlers JSON: ~/.config/cloudmount/previous_handlers.json
    fn previous_handlers_path() -> cloudmount_core::Result<PathBuf> {
        cloudmount_core::config::config_dir().map(|d| d.join(PREVIOUS_HANDLERS_FILE))
    }
    ...
    fn save_previous_handlers(handlers: &HashMap<String, String>) -> cloudmount_core::Result<()> {
        let path = previous_handlers_path()?;
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                cloudmount_core::Error::Config(format!(
        ...
        let json = serde_json::to_string_pretty(handlers).map_err(|e| {
            cloudmount_core::Error::Config(format!("failed to serialize previous handlers: {e}"))
        })?;
    ...
    fn xdg_set_default(desktop_file: &str, mime_type: &str) -> cloudmount_core::Result<()> {
        let status = std::process::Command::new("xdg-mime")
            .args(["default", desktop_file, mime_type])
            .status()
            .map_err(|e| {
                cloudmount_core::Error::Config(format!("failed to run xdg-mime default: {e}"))
            })?;
        if !status.success() {
            return Err(cloudmount_core::Error::Config(format!(
    ...
    fn desktop_file_content(exe_path: &str) -> String {
        format!(
            "[Desktop Entry]\n\
             Type=Application\n\
             Name=CloudMount File Handler\n\
             Comment=Opens Office files through CloudMount for online collaboration\n\
    ...
    /// Register CloudMount as the handler for Office file types on Linux.
    ///
    ...
    pub fn register() -> cloudmount_core::Result<()> {
        let exe_path = std::env::current_exe().map_err(|e| {
            cloudmount_core::Error::Config(format!("failed to get current exe path: {e}"))
        })?;
        let exe_str = exe_path.to_string_lossy();
    ...
    /// Unregister CloudMount file associations and restore previous handlers.
    pub fn unregister() -> cloudmount_core::Result<()> {
    ...
    /// Check if CloudMount file associations are currently registered.
    ///
    /// Returns `true` if the .desktop file exists AND CloudMount is the
...
mod macos {
    ...
    const BUNDLE_ID: &str = "com.cloudmount";

    /// JSON file where we store previous handlers for restoration.
    const PREVIOUS_HANDLERS_FILE: &str = "previous_handlers.json";

    /// Path to our previous handlers JSON: ~/Library/Application Support/cloudmount/previous_handlers.json
    fn previous_handlers_path() -> cloudmount_core::Result<PathBuf> {
        cloudmount_core::config::config_dir().map(|d| d.join(PREVIOUS_HANDLERS_FILE))
    }
    ...
    /// Register CloudMount as the handler for Office file types on macOS.
    ///
    ...
    pub fn register() -> cloudmount_core::Result<()> {
        if !has_duti() {
            ...
            return Err(cloudmount_core::Error::Config(
    ...
    /// Check if CloudMount file associations are currently registered.
    ///
    /// Returns `true` if CloudMount is the default handler for at least one
    /// Office extension.
    pub fn is_registered() -> bool {
        if !has_duti() {
            return false;
