...
async fn error_429_retries_then_fails() {
    ...
    match err {
        carminedesktop_core::Error::GraphApi { status, .. } => assert_eq!(status, 429),
...
async fn check_drive_exists_returns_404_error() {
    ...
    match err {
        carminedesktop_core::Error::GraphApi { status, .. } => assert_eq!(status, 404),
...
async fn check_drive_exists_returns_403_error() {
    ...
    match err {
        carminedesktop_core::Error::GraphApi { status, .. } => assert_eq!(status, 403),
...
async fn poll_copy_status_returns_completed() {
    ...
    match status {
        carminedesktop_graph::CopyStatus::Completed { resource_id } => {
...
async fn poll_copy_status_returns_in_progress() {
    ...
    match status {
        carminedesktop_graph::CopyStatus::InProgress { percentage } => {
...
async fn poll_copy_status_returns_failed() {
    ...
    match status {
        carminedesktop_graph::CopyStatus::Failed { message } => {
...
async fn poll_copy_status_sends_no_auth_header() {
    ...
    assert!(matches!(
        status,
        carminedesktop_graph::CopyStatus::Completed { .. }
    ));
...
async fn handle_error_maps_423_to_locked() {
    ...
    assert!(
        matches!(err, carminedesktop_core::Error::Locked),
