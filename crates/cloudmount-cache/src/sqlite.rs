...
impl SqliteStore {
    pub fn open(path: &std::path::Path) -> cloudmount_core::Result<Self> {
    ...
    fn create_tables(&self) -> cloudmount_core::Result<()> {
...
",
    ...
    pub fn upsert_item(
        ...
        let json = serde_json::to_string(item)
            .map_err(|e| cloudmount_core::Error::Cache(format!("serialize failed: {e}")))?;
        ...
        .map_err(|e| cloudmount_core::Error::Cache(format!("upsert failed: {e}")))?;
    ...
    pub fn get_item_by_id(
        ...
        let result = stmt
            ...
            .optional()
            .map_err(|e| cloudmount_core::Error::Cache(format!("query failed: {e}")))?;

        match result {
            Some((inode, json)) => {
                let item: DriveItem = serde_json::from_str(&json).map_err(|e| {
                    cloudmount_core::Error::Cache(format!("deserialize failed: {e}"))
                })?;
                Ok(Some((inode, item)))
            }
            None => Ok(None),
    ...
    pub fn get_children(
        ...
        for row in rows {
            let (inode, json) =
                row.map_err(|e| cloudmount_core::Error::Cache(format!("row read failed: {e}")))?;
            let item: DriveItem = serde_json::from_str(&json)
                .map_err(|e| cloudmount_core::Error::Cache(format!("deserialize failed: {e}")))?;
    ...
    pub fn delete_item(&self, item_id: &str) -> cloudmount_core::Result<()> {
        ...
        conn.execute("DELETE FROM items WHERE item_id = ?1", params![item_id])
            .map_err(|e| cloudmount_core::Error::Cache(format!("delete failed: {e}")))?;
        Ok(())
    }
    ...
    pub fn set_delta_token(&self, drive_id: &str, token: &str) -> cloudmount_core::Result<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute(
        ...
        .map_err(|e| cloudmount_core::Error::Cache(format!("set delta token failed: {e}")))?;
        Ok(())
    }
    ...
    pub fn apply_delta(
        ...
        let tx = conn
            ...
            .map_err(|e| cloudmount_core::Error::Cache(format!("transaction failed: {e}")))?;
        ...
        for (inode, item, parent_inode) in items {
            ...
            .map_err(|e| cloudmount_core::Error::Cache(format!("upsert in delta failed: {e}")))?;
        }

        for id in deleted_ids {
            tx.execute("DELETE FROM items WHERE item_id = ?1", params![id])
                .map_err(|e| {
                    cloudmount_core::Error::Cache(format!("delete in delta failed: {e}"))
                })?;
        }
        ...
        tx.commit()
            .map_err(|e| cloudmount_core::Error::Cache(format!("commit failed: {e}")))?;
