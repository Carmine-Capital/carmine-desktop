...
impl DiskCache {
    ...
    pub async fn put(
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)
                .await
                .map_err(|e| filesync_core::Error::Cache(format!("mkdir failed: {e}")))?;
        }
        fs::write(&path, content)
            .await
            .map_err(|e| filesync_core::Error::Cache(format!("write cache failed: {e}")))?;
    ...
    pub async fn remove(&self, drive_id: &str, item_id: &str) -> filesync_core::Result<()> {
        let path = self.content_path(drive_id, item_id);
        if path.exists() {
            fs::remove_file(&path)
                .await
                .map_err(|e| filesync_core::Error::Cache(format!("remove cache failed: {e}")))?;
        }
        if let Ok(conn) = self.tracker.lock() {
            let _ = conn.execute(
    ...
    pub async fn clear(&self) -> filesync_core::Result<()> {
        if self.base_dir.exists() {
            fs::remove_dir_all(&self.base_dir)
                .await
                .map_err(|e| filesync_core::Error::Cache(format!("clear cache failed: {e}")))?;
            fs::create_dir_all(&self.base_dir).await.map_err(|e| {
                filesync_core::Error::Cache(format!("recreate cache dir failed: {e}"))
            })?;
        }
        if let Ok(conn) = self.tracker.lock() {
    ...
    async fn evict_if_needed(&self) -> filesync_core::Result<()> {
        ...
        let entries = {
            let conn = self
                .tracker
                .lock()
                .map_err(|e| filesync_core::Error::Cache(format!("tracker lock failed: {e}")))?;
            let mut stmt = conn
                .prepare(
                    "SELECT drive_id, item_id, file_size FROM cache_entries ORDER BY last_access ASC",
                )
                .map_err(|e| {
                    filesync_core::Error::Cache(format!("prepare eviction query failed: {e}"))
                })?;
            ...
            let rows = stmt
                ...
                .map_err(|e| filesync_core::Error::Cache(format!("eviction query failed: {e}")))?;
