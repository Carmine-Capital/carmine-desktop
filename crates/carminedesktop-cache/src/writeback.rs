...
impl WriteBackBuffer {
    ...
    pub async fn write_chunk(
        ...
        if let Some(parent) = tmp_path.parent() {
            fs::create_dir_all(parent).await.map_err(|e| {
                carminedesktop_core::Error::Cache(format!("mkdir pending failed: {e}"))
            })?;
        }
        let mut file = tokio::fs::OpenOptions::new()
            ...
            .await
            .map_err(|e| {
                carminedesktop_core::Error::Cache(format!("open chunk file failed: {e}"))
            })?;
        file.seek(std::io::SeekFrom::Start(offset))
            .await
            .map_err(|e| carminedesktop_core::Error::Cache(format!("seek chunk failed: {e}")))?;
        file.write_all(chunk)
            .await
            .map_err(|e| carminedesktop_core::Error::Cache(format!("write chunk failed: {e}")))?;
        Ok(())
    }
    ...
    pub async fn finish_chunked_write(
        ...
        fs::rename(&tmp_path, &path).await.map_err(|e| {
            carminedesktop_core::Error::Cache(format!("rename pending failed: {e}"))
        })?;
        Ok(())
    }
    ...
    pub async fn persist(&self, drive_id: &str, item_id: &str) -> carminedesktop_core::Result<()> {
        let key = Self::buffer_key(drive_id, item_id);
        ...
        fs::write(&tmp_path, &content).await.map_err(|e| {
            carminedesktop_core::Error::Cache(format!("persist pending failed: {e}"))
        })?;
        fs::rename(&tmp_path, &path).await.map_err(|e| {
            carminedesktop_core::Error::Cache(format!("rename pending failed: {e}"))
        })?;
    ...
    pub async fn list_pending(&self) -> carminedesktop_core::Result<Vec<(String, String)>> {
        ...
        while let Some(drive_entry) = drive_dirs.next_entry().await.map_err(|e| {
            carminedesktop_core::Error::Cache(format!("read drive entry failed: {e}"))
        })? {
            let drive_id = drive_entry.file_name().to_string_lossy().to_string();
            let mut item_files = fs::read_dir(drive_entry.path()).await.map_err(|e| {
                carminedesktop_core::Error::Cache(format!("read item dir failed: {e}"))
            })?;

            while let Some(item_entry) = item_files.next_entry().await.map_err(|e| {
                carminedesktop_core::Error::Cache(format!("read item entry failed: {e}"))
            })? {
