...
const SERVICE_NAME: &str = "cloudmount";
const SALT_LEN: usize = 16;
const NONCE_LEN: usize = 12;

pub fn store_tokens(account_id: &str, tokens: &TokenResponse) -> cloudmount_core::Result<()> {
...
pub fn load_tokens(account_id: &str) -> cloudmount_core::Result<Option<TokenResponse>> {
...
pub fn delete_tokens(account_id: &str) -> cloudmount_core::Result<()> {
...
fn serialize_tokens(tokens: &TokenResponse) -> cloudmount_core::Result<String> {
    ...
    serde_json::to_string(&payload)
        .map_err(|e| cloudmount_core::Error::Auth(format!("failed to serialize tokens: {e}")))
}

fn deserialize_tokens(data: &str) -> cloudmount_core::Result<TokenResponse> {
    let value: serde_json::Value = serde_json::from_str(data)
        .map_err(|e| cloudmount_core::Error::Auth(format!("failed to parse stored tokens: {e}")))?;
...
fn load_tokens_keyring(account_id: &str) -> cloudmount_core::Result<Option<TokenResponse>> {
...
fn derive_key(password: &[u8], salt: &[u8]) -> cloudmount_core::Result<Zeroizing<[u8; 32]>> {
    let params = Params::new(64 * 1024, 3, 1, Some(32))
        .map_err(|e| cloudmount_core::Error::Auth(format!("argon2 params error: {e}")))?;
    let argon2 = Argon2::new(Algorithm::Argon2id, Version::V0x13, params);

    let mut key = Zeroizing::new([0u8; 32]);
    argon2
        .hash_password_into(password, salt, key.as_mut())
        .map_err(|e| cloudmount_core::Error::Auth(format!("key derivation failed: {e}")))?;
    Ok(key)
}
...
fn machine_password() -> Vec<u8> {
    let username = std::env::var("USER")
        ...
        .unwrap_or_else(|_| "cloudmount".to_string());
...
fn store_tokens_encrypted(account_id: &str, serialized: &str) -> cloudmount_core::Result<()> {
    ...
    let ciphertext = cipher
        ...
        .map_err(|_| cloudmount_core::Error::Auth("encryption failed".to_string()))?;
...
fn load_tokens_encrypted(account_id: &str) -> cloudmount_core::Result<Option<TokenResponse>> {
    ...
    let data = std::fs::read(&path)
        .map_err(|e| cloudmount_core::Error::Auth(format!("read token file failed: {e}")))?;
    ...
    let plaintext = cipher.decrypt(nonce, ciphertext).map_err(|_| {
        cloudmount_core::Error::Auth("decryption failed (corrupted or wrong machine)".to_string())
    })?;
