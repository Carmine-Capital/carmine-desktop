...
pub async fn run_pkce_flow(
    ...
    let listener = tokio::net::TcpListener::bind(format!("127.0.0.1:{port}"))
        .await
        .map_err(|e| {
            cloudmount_core::Error::Auth(format!("failed to bind callback listener: {e}"))
        })?;

    let actual_port = listener
        .local_addr()
        .map_err(|e| cloudmount_core::Error::Auth(format!("failed to get listener address: {e}")))?
        .port();
    ...
    let mut auth_url = Url::parse(&format!("{}/authorize", authority_url(tenant_id)))
        .map_err(|e| cloudmount_core::Error::Auth(format!("invalid authority URL: {e}")))?;
...
async fn wait_for_callback(listener: tokio::net::TcpListener) -> cloudmount_core::Result<String> {
    ...
    let code = rx
        .await
        .map_err(|_| cloudmount_core::Error::Auth("callback channel closed".into()))?;

    if code.is_empty() {
        return Err(cloudmount_core::Error::Auth(
...
pub async fn exchange_code(
    ...
    verifier: &str,
    port: u16,
) -> cloudmount_core::Result<TokenResponse> {
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| cloudmount_core::Error::Auth(format!("token exchange request failed: {e}")))?;

    if !resp.status().is_success() {
        let body = resp.text().await.unwrap_or_default();
        return Err(cloudmount_core::Error::Auth(format!(
            "token exchange failed: {body}"
        )));
    }

    let body: serde_json::Value = resp.json().await.map_err(|e| {
        cloudmount_core::Error::Auth(format!("failed to parse token response: {e}"))
    })?;
...
pub async fn refresh_token(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| cloudmount_core::Error::Auth(format!("token refresh request failed: {e}")))?;
...
fn parse_token_response(body: &serde_json::Value) -> cloudmount_core::Result<TokenResponse> {
    let access_token = body["access_token"]
        .as_str()
        .ok_or_else(|| cloudmount_core::Error::Auth("missing access_token in response".into()))?
        .to_string();

    let refresh = body["refresh_token"]
        .as_str()
        .ok_or_else(|| cloudmount_core::Error::Auth("missing refresh_token in response".into()))?
        .to_string();
