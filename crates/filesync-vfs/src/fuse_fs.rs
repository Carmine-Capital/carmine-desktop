...
impl FileSyncFs {
    ...
    pub fn mount(self, mountpoint: &str) -> filesync_core::Result<fuser::BackgroundSession> {
        let mut config = Config::default();
        config.mount_options = vec![
            MountOption::RW,
            MountOption::FSName("filesync".to_string()),
            MountOption::AutoUnmount,
        ];

        fuser::spawn_mount2(self, mountpoint, &config)
            .map_err(|e| filesync_core::Error::Filesystem(format!("mount failed: {e}")))
    }
...
impl Filesystem for FileSyncFs {
