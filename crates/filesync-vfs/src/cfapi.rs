...
const PROVIDER_NAME: &str = "FileSync";
...
pub struct FileSyncCfFilter {
    core: CoreOps,
    mount_path: PathBuf,
}

impl FileSyncCfFilter {
    pub fn new(
...
impl SyncFilter for FileSyncCfFilter {
    fn fetch_data(
        &self,
        request: Request,
...
fn build_sync_root_id() -> filesync_core::Result<SyncRootId> {
    let sid = SecurityId::current_user()
        .map_err(|e| filesync_core::Error::Filesystem(format!("failed to get user SID: {e:?}")))?;
    Ok(SyncRootIdBuilder::new(PROVIDER_NAME)
        .user_security_id(sid)
        .build())
}

fn ensure_mount_dir(path: &Path) -> filesync_core::Result<()> {
    if !path.exists() {
        std::fs::create_dir_all(path).map_err(|e| {
            filesync_core::Error::Filesystem(format!(
...
fn register_sync_root(sync_root_id: &SyncRootId, mount_path: &Path) -> filesync_core::Result<()> {
    let info = SyncRootInfo::default()
        ...
        .map_err(|e| filesync_core::Error::Filesystem(format!("sync root path invalid: {e:?}")))?;

    sync_root_id.register(info).map_err(|e| {
        filesync_core::Error::Filesystem(format!("sync root registration failed: {e:?}"))
    })?;
...
pub struct CfMountHandle {
    ...
    _connection: Connection<FileSyncCfFilter>,
...
impl CfMountHandle {
    pub fn mount(
        ...
        let is_registered = sync_root_id.is_registered().map_err(|e| {
            filesync_core::Error::Filesystem(format!("sync root registration check failed: {e:?}"))
        })?;
        ...
        let filter = FileSyncCfFilter::new(
            graph.clone(),
            cache.clone(),
            inodes,
            drive_id.clone(),
        ...
        let connection = Session::new().connect(mount_path, filter).map_err(|e| {
            filesync_core::Error::Filesystem(format!("CfApi connect failed: {e:?}"))
        })?;
    ...
    pub fn unmount(self) -> filesync_core::Result<()> {
        ...
        self.sync_root_id.unregister().map_err(|e| {
            filesync_core::Error::Filesystem(format!("sync root unregister failed: {e:?}"))
        })?;
