...
use carminedesktop_cache::writeback::WriteBackBuffer;
use carminedesktop_core::config::{EffectiveConfig, UserConfig};
...
fn unique_temp_dir(prefix: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!("carminedesktop-integration-{prefix}-{id}"))
}
...
async fn test_e2e_onedrive_mount_list_read_write_unmount() -> carminedesktop_core::Result<()> {
...
#[tokio::test]
#[ignore = "requires live Graph API"]
async fn test_e2e_sharepoint_browse_mount_read_unmount() -> carminedesktop_core::Result<()> {
...
#[tokio::test]
async fn test_offline_cached_files_readable() -> carminedesktop_core::Result<()> {
...
#[tokio::test]
async fn test_write_conflict_creates_conflict_copy() -> carminedesktop_core::Result<()> {
    ...
    item.parent_reference = Some(carminedesktop_core::types::ParentReference {
...
#[tokio::test]
async fn test_cache_eviction_lru() -> carminedesktop_core::Result<()> {
...
#[tokio::test]
async fn test_crash_recovery_pending_writes() -> carminedesktop_core::Result<()> {
...
#[tokio::test]
async fn test_initialization_sequence() -> carminedesktop_core::Result<()> {
    ...
    let cache_dir = base.join("cache");
    let db_path = base.join("carminedesktop.db");
    std::fs::create_dir_all(&cache_dir)?;
    let cache = Arc::new(CacheManager::new(
        cache_dir,
        db_path,
...
#[tokio::test]
async fn test_token_restoration_flow() -> carminedesktop_core::Result<()> {
    use carminedesktop_auth::AuthManager;
    use carminedesktop_vfs::inode::InodeTable;
...
#[tokio::test]
async fn test_sign_in_onedrive_discovery_and_mount_config() -> carminedesktop_core::Result<()> {
    use carminedesktop_core::config::derive_mount_point;
    ...
    user_config
        .accounts
        .push(carminedesktop_core::config::AccountMetadata {
            id: drive.id.clone(),
            email: None,
...
#[tokio::test]
async fn test_sign_out_clears_account_and_config() -> carminedesktop_core::Result<()> {
    use carminedesktop_vfs::inode::InodeTable;
    ...
    user_config.add_onedrive_mount(
        "drive-to-remove",
        "/tmp/carminedesktop-test-signout/OneDrive",
        None,
    )?;
...
#[tokio::test]
async fn test_account_scoped_mounts_filtered() -> carminedesktop_core::Result<()> {
    let mut user_config = UserConfig::load("")?;
...
#[tokio::test]
async fn test_crash_recovery_reupload_pending_writes() -> carminedesktop_core::Result<()> {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn test_graceful_shutdown_stops_sync_and_mounts() -> carminedesktop_core::Result<()> {
    use carminedesktop_vfs::inode::InodeTable;
...
#[tokio::test]
async fn test_auth_degradation_detection_in_delta_sync() -> carminedesktop_core::Result<()> {
    use carminedesktop_vfs::inode::InodeTable;
    ...
    let graph = Arc::new(GraphClient::with_base_url(server.uri(), || async {
        Err(carminedesktop_core::Error::Auth(
    ...
    let result = carminedesktop_cache::sync::run_delta_sync(
        &graph,
        &cache,
        "test-drive",
        &inode_allocator,
    ...
    let result2 = carminedesktop_cache::sync::run_delta_sync(
        &graph_ok,
        &cache,
        "test-drive",
        &inode_allocator2,
...
async fn test_smoke_windows_winfsp_mount_list_read_write_unmount() -> carminedesktop_core::Result<()>
{
    ...
    let base = std::env::temp_dir().join(format!("carminedesktop-smoke-win-{test_id}"));
    let cache_dir = base.join("cache");
    let db_path = base.join("metadata.db");
...
#[tokio::test(flavor = "multi_thread")]
async fn test_surgical_invalidation_create_preserves_parent_cache()
...
-> carminedesktop_core::Result<()> {
    ...
    let result1 = carminedesktop_cache::sync::run_delta_sync(
        &graph,
        &cache,
        "test-drive",
        &inode_allocator,
...
#[tokio::test]
async fn test_delta_sync_result_contains_deleted_items_with_path() -> carminedesktop_core::Result<()>
{
    use carminedesktop_vfs::inode::InodeTable;
    ...
    let resolved = carminedesktop_cache::resolve_deleted_path(deleted);
    assert_eq!(
        resolved,
...
#[tokio::test]
async fn test_delta_sync_result_empty_when_no_changes() -> carminedesktop_core::Result<()> {
    use carminedesktop_vfs::inode::InodeTable;
