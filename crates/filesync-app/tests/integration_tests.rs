...
use filesync_cache::disk::DiskCache;
use filesync_cache::writeback::WriteBackBuffer;
use filesync_core::config::{EffectiveConfig, PackagedDefaults, UserConfig};
...
fn unique_temp_dir(prefix: &str) -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!("filesync-integration-{prefix}-{id}"))
}
...
async fn test_e2e_onedrive_mount_list_read_write_unmount() -> filesync_core::Result<()> {
...
#[tokio::test]
#[ignore = "requires live Graph API"]
async fn test_e2e_sharepoint_browse_mount_read_unmount() -> filesync_core::Result<()> {
...
#[tokio::test]
async fn test_offline_cached_files_readable() -> filesync_core::Result<()> {
...
#[tokio::test]
async fn test_write_conflict_creates_conflict_copy() -> filesync_core::Result<()> {
    ...
    item.parent_reference = Some(filesync_core::types::ParentReference {
...
#[tokio::test]
async fn test_cache_eviction_lru() -> filesync_core::Result<()> {
...
#[tokio::test]
async fn test_crash_recovery_pending_writes() -> filesync_core::Result<()> {
...
#[test]
fn test_preconfigured_build_loads_packaged_defaults() -> filesync_core::Result<()> {
    let packaged_toml = r#"
[tenant]
id = "org-tenant-12345"
...
"#;
    ...
    assert_eq!(packaged.app_name(), "Contoso FileSync");
    ...
    assert_eq!(effective.app_name, "Contoso FileSync");
...
#[test]
fn test_update_preserves_user_overrides() -> filesync_core::Result<()> {
...
#[cfg(target_os = "macos")]
async fn test_smoke_macos_fuse_mount_list_read_write_unmount() -> filesync_core::Result<()> {
    ...
    let base = std::env::temp_dir().join(format!("filesync-smoke-macos-{test_id}"));
    let cache_dir = base.join("cache");
    let db_path = base.join("metadata.db");
...
#[tokio::test]
async fn test_initialization_sequence() -> filesync_core::Result<()> {
    ...
    assert_eq!(packaged.app_name(), "FileSync");
    assert!(!packaged.has_packaged_config());
    ...
    let cache_dir = base.join("cache");
    let db_path = base.join("filesync.db");
    std::fs::create_dir_all(&cache_dir)?;
    let cache = Arc::new(CacheManager::new(
        cache_dir,
        db_path,
...
#[tokio::test]
async fn test_token_restoration_flow() -> filesync_core::Result<()> {
    use filesync_auth::AuthManager;
    use filesync_vfs::inode::InodeTable;
...
#[tokio::test]
async fn test_sign_in_onedrive_discovery_and_mount_config() -> filesync_core::Result<()> {
    use filesync_core::config::derive_mount_point;
    ...
    user_config
        .accounts
        .push(filesync_core::config::AccountMetadata {
            id: drive.id.clone(),
            email: None,
...
#[tokio::test]
async fn test_sign_out_clears_account_and_config() -> filesync_core::Result<()> {
    use filesync_vfs::inode::InodeTable;
    ...
    user_config.add_onedrive_mount("drive-to-remove", "/tmp/filesync-test-signout/OneDrive")?;
...
#[tokio::test]
async fn test_crash_recovery_reupload_pending_writes() -> filesync_core::Result<()> {
    let server = MockServer::start().await;
...
#[tokio::test]
async fn test_graceful_shutdown_stops_sync_and_mounts() -> filesync_core::Result<()> {
    use filesync_vfs::inode::InodeTable;
...
#[tokio::test]
async fn test_auth_degradation_detection_in_delta_sync() -> filesync_core::Result<()> {
    use filesync_vfs::inode::InodeTable;
    ...
    let result =
        filesync_cache::sync::run_delta_sync(&graph, &cache, "test-drive", &inode_allocator).await;

    // Simulate the auth degradation detection logic from start_delta_sync:
    match result {
        Err(filesync_core::Error::Auth(ref msg)) if msg.contains("re-authentication required") => {
    ...
    let result2 =
        filesync_cache::sync::run_delta_sync(&graph_ok, &cache, "test-drive", &inode_allocator2)
            .await;
    assert!(
        result2.is_ok(),
...
async fn test_smoke_windows_cfapi_mount_list_read_write_unmount() -> filesync_core::Result<()> {
    use filesync_vfs::CfMountHandle;
    use filesync_vfs::inode::InodeTable;
    ...
    let base = std::env::temp_dir().join(format!("filesync-smoke-win-{test_id}"));
    let cache_dir = base.join("cache");
    let db_path = base.join("metadata.db");
