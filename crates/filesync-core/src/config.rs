...
const DEFAULT_APP_NAME: &str = "FileSync";
...
pub fn config_dir() -> PathBuf {
    dirs::config_dir()
        .map(|d| d.join("filesync"))
        .unwrap_or_else(|| PathBuf::from(".filesync"))
}
...
pub fn cache_dir() -> PathBuf {
    dirs::cache_dir()
        .map(|d| d.join("filesync"))
        .unwrap_or_else(|| PathBuf::from(".filesync-cache"))
}
...
pub mod autostart {
    ...
    fn enable(app_path: &str) -> crate::Result<()> {
        ...
        std::fs::write(service_dir.join("filesync.service"), unit)?;

        std::process::Command::new("systemctl")
            .args(["--user", "enable", "filesync.service"])
            .output()
    ...
    fn disable() -> crate::Result<()> {
        let _ = std::process::Command::new("systemctl")
            .args(["--user", "disable", "filesync.service"])
            .output();

        let service_path = dirs::config_dir().map(|d| d.join("systemd/user/filesync.service"));
...
<dict>
    <key>Label</key>
    <string>com.filesync.agent</string>
    ...
    <false/>
        ...
        std::fs::write(plist_dir.join("com.filesync.agent.plist"), plist)?;
        Ok(())
    }
    ...
    fn disable() -> crate::Result<()> {
        let plist_path =
            dirs::home_dir().map(|d| d.join("Library/LaunchAgents/com.filesync.agent.plist"));
